...
def fit_power_law_rating_curve_for_years(
    df: pd.DataFrame,
    years: Sequence[int] | None = None,
    q_rel_unc: float = 0.10,
    wl_abs_unc: float = 2.0,
    h0_fixed: float | None = None,
    min_q_positive: float = 1e-6,
) -> tuple[RatingCurveParams, pd.DataFrame]:
    """
    Fit a rating curve using only data from the specified years.

    Parameters
    ----------
    df : DataFrame
        Must have columns: 'datetime', 'wl_final', 'discharge', 'year'.
    years : sequence of int or None
        Years to include in this rating curve. If None, use all years.
    q_rel_unc : float
        Relative Q uncertainty (e.g. 0.10 for 10%).
    wl_abs_unc : float
        Absolute wl uncertainty in same units as wl_final (e.g. 2 cm).
    h0_fixed, min_q_positive :
        Passed through to fit_power_law_rating_curve.

    Returns
    -------
    params : RatingCurveParams
    df_sub : DataFrame
        Subset of df actually used in the fit, with added columns:
        'q_sigma', 'wl_sigma', and 'q_fit'.
    """
    ...
    wl = df_sub["wl_final"].to_numpy(dtype=float)
    q  = df_sub["discharge"].to_numpy(dtype=float)

    # uncertainties
    q_sigma = q_rel_unc * q
    wl_sigma = np.full_like(wl, wl_abs_unc, dtype=float)

    # fit
    params = fit_power_law_rating_curve(
        wl=wl,
        q=q,
        q_sigma=q_sigma,
        h0_fixed=h0_fixed,
        min_q_positive=min_q_positive,
    )

    # predicted Q for diagnostics
    df_sub["q_sigma"] = q_sigma
    df_sub["wl_sigma"] = wl_sigma
    df_sub["q_fit"] = predict_discharge(df_sub["wl_final"].to_numpy(), params)

    return params, df_sub
...
def plot_data_with_q_errorbars_by_year(df):
    ...
    plt.xlabel("Water level h (wl_final)")
    plt.ylabel("Discharge Q")
    plt.title("Stage–discharge with Q uncertainty, colored by year")
    plt.grid(True)
...
def plot_rating_curve(wl, q, params, q_sigma=None):
    ...
    plt.figure()
    if q_sigma is not None:
        plt.errorbar(wl, q, yerr=q_sigma, fmt='o', capsize=3, alpha=0.7, label=f"Q (with uncertainty)")
    else:
    ...
    plt.xlabel("Water level h (wl_final)")
    plt.ylabel("Discharge Q [$m^3 s^{-1}$]")
...
# wl_q_df["datetime"] = pd.to_datetime(wl_q_df["datetime"], utc=True)
wl_q_df["year"] = wl_q_df["datetime"].dt.year

# Ensure datetime/year columns exist
wl_q_df["datetime"] = pd.to_datetime(wl_q_df["datetime"], utc=True)
wl_q_df["year"] = wl_q_df["datetime"].dt.year

# One curve for 2023
params_23, df_23 = fit_power_law_rating_curve_for_years(
    wl_q_df,
    years=[2023],
    q_rel_unc=0.30,
    wl_abs_unc=4.0,
...
# One curve for 2024
params_24, df_24 = fit_power_law_rating_curve_for_years(
    wl_q_df,
    years=[2024],
    q_rel_unc=0.30,
    wl_abs_unc=4.0,
...
# Another curve for 2025 only
params_25, df_25 = fit_power_law_rating_curve_for_years(
    wl_q_df,
    years=[2025],
    q_rel_unc=0.30,
    wl_abs_unc=4.0,
...
# Plot for 2023
plot_data_with_q_errorbars(df_23["wl_final"], df_23["discharge"], df_23["q_sigma"])
plot_loglog_fit(df_23["wl_final"], df_23["discharge"], params_23, q_sigma=df_23["q_sigma"])
plot_rating_curve(df_23["wl_final"], df_23["discharge"], params_23, q_sigma=df_23["q_sigma"])

# Plot for 2024
plot_data_with_q_errorbars(df_24["wl_final"], df_24["discharge"], df_24["q_sigma"])
plot_loglog_fit(df_24["wl_final"], df_24["discharge"], params_24, q_sigma=df_24["q_sigma"])
plot_rating_curve(df_24["wl_final"], df_24["discharge"], params_24, q_sigma=df_24["q_sigma"])

# Plot for 2025
plot_data_with_q_errorbars(df_25["wl_final"], df_25["discharge"], df_25["q_sigma"])
plot_loglog_fit(df_25["wl_final"], df_25["discharge"], params_25, q_sigma=df_25["q_sigma"])
plot_rating_curve(df_25["wl_final"], df_25["discharge"], params_25, q_sigma=df_25["q_sigma"])
